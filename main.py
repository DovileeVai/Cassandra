...
def create_app():
    ...
    def delete_channel(channelId):
        ...
        if exists:
            query2 = "DELETE FROM chat_app.messages_by_channel WHERE channel_id = %s"
            session.execute(query2, (channelId,))

            query3 = "DELETE FROM chat_app.members WHERE channel_id = %s"
            session.execute(query3, (channelId,))

            query = "DELETE FROM chat_app.channels WHERE id = %s"
